...
public class EncodingMain2 {
    ...
    public static void main(String[] args) {
        ...
        test("A", US_ASCII, UTF_16BE); // UTF_16BE 디코딩 실패 -> �

        System.out.println("== 한글 인코딩 - 기본 ==");
        test("가", US_ASCII, US_ASCII); // "가" > 63 > ?(ascii set 63: ?)
        test("가", ISO_8859_1, ISO_8859_1);
        test("가", EUC_KR, EUC_KR); // 가 >> [-80, -95] 2byte >> 가
        test("가", MS_949, MS_949); // 2byte 가
        test("가", UTF_8, UTF_8); // [-22, -80, -128] 3byte 가
        test("가", UTF_16BE, UTF_16BE); // [-84, 0] 2byte 가

        System.out.println("== 한글 인코딩 - 디코딩이 다른 경우 ==");
        test("가", EUC_KR, MS_949);
        test("뷁", MS_949, EUC_KR); // x
        test("가", EUC_KR, UTF_8); // x
        test("가", MS_949, UTF_8); // x
        test("가", UTF_8, UTF_8); // x
